...
mirage_cache_fmstr = 'MIRAGE_CACHE {name}("{name}", {frequency}, {fill_level}, {sets}, {ways}, {skews}, {extra_tags}, {cuckoo_height}, {wq_size}, {rq_size}, {pq_size}, {mshr_size}, {hit_latency}, {fill_latency}, {max_read}, {max_write}, {offset_bits}, {prefetch_as_load:b}, {wq_check_full_addr:b}, {virtual_prefetch:b}, {prefetch_activate_mask}, {lower_level}, MIRAGE_CACHE::pref_t::{prefetcher_name}, MIRAGE_CACHE::repl_t::{replacement_name});\n'
...
default_llc  = { 'sets': 2048*config_file['num_cores'], 'ways': 14, 'skews': 2, 'extra_tags': 6, 'cuckoo_height': 3, 'rq_size': 32*config_file['num_cores'], 'wq_size': 32*config_file['num_cores'], 'pq_size': 32*config_file['num_cores'], 'mshr_size': 64*config_file['num_cores'], 'latency': 20, 'fill_latency': 1, 'max_read': config_file['num_cores'], 'max_write': config_file['num_cores'], 'prefetch_as_load': False, 'virtual_prefetch': False, 'wq_check_full_addr': False, 'prefetch_activate': 'LOAD,PREFETCH', 'prefetcher': 'no', 'replacement': 'lru', 'name': 'LLC', 'lower_level': 'DRAM' }
...
with open(instantiation_file_name, 'wt') as wfp:
    ...
    wfp.write('#include "mirage_cache.h"\n')
    ...
    for elem in memory_system:
        if 'pscl5_set' in elem:
            ...
            wfp.write(mirage_cache_fmstr.format(**elem))
        else:
    ...
    wfp.write('std::array<CACHE*, NUM_CACHES-1> caches {\n')
    wfp.write(', '.join('&{name}'.format(**elem) for elem in memory_system if ('pscl5_set' not in elem) and ('skews' not in elem)))
    wfp.write('\n};\n')
    
    wfp.write('MIRAGE_CACHE* llc_cache = &{name};\n'.format(**memory_system[0]))
